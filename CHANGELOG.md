...
- **Phase 4.1 — Distiller LLM wiring** (`src/ctxmtg/farming/distiller.py`).
  When a `farming`-role LLM is configured, the `DistillerStage` now
  replaces the mechanical per-entity summary string with a one-sentence
  natural-language summary generated by the LLM, using the structured
  entity profile (name, type, mention count, interaction count, top
  predicates, co-entities) as context. The deterministic
  `_build_summary` output is the unconditional fallback for any LLM
  error, empty response, `INSUFFICIENT` sentinel, or unavailable
...
- **Ingest LLM parity for HTTP API** (`src/ctxmtg/web/routes/ingest.py`).
  ...
  programmatic ingestion (proxy, browser extension, inbox watcher,
  webhooks) gets the same enhancement that interactive CLI ingestion
  gets. Both code paths follow the same fallback pattern: missing /
  misconfigured / unreachable provider degrades silently to the
  deterministic Phase-1 pipeline.
- **Abstractive interaction summaries**
  (`src/ctxmtg/extraction/pipeline.py`). `BasicExtractionPipeline`
  takes a new optional `llm` parameter; when supplied, the
  TextRank summary is replaced by a 2-3 sentence narrative summary
  generated by the LLM. The TextRank string is the unconditional
  fallback. The prompt explicitly forbids inventing facts and uses
  the `INSUFFICIENT` sentinel for thin inputs. The summary is a
  description string only -- it never enters the entities/facts
  tables. Long source text is hard-capped before being sent to the
  prompt.
- **Distiller summaries on the entity page**
  (`src/ctxmtg/web/routes/entities.py`,
  `src/ctxmtg/web/templates/entities.html`). New "Top Distilled
  Entities" card surfaces the highest-relevance entities and their
  Distiller-written summaries -- the user-visible payoff of Phase
  4.1. Read-only view; gracefully empty until at least one farming
  cycle has run.
- **LLM role status widget on the dashboard**
  (`src/ctxmtg/web/routes/dashboard.py`,
  `src/ctxmtg/web/templates/dashboard.html`). One-glance "is the
  genie wired up?" widget showing each of the 6 pipeline roles as
  a green / amber badge. Reads the per-role config from `.env`;
  does not perform a live ping (cost / latency).

### Documentation

- **README "For LLMs and AI Agents" section refreshed** with new
  invariants: the LLM is an enhancer never a source of truth;
  every `.generate()` call must have a deterministic fallback;
